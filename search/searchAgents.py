...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        """
        currentPosition,visitedCorners=state[0],list(state[1])
        successors = []
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                successor=((nextPosition,tuple(visitedCorners)),action,1)
                if nextPosition in self.corners and nextPosition not in visitedCorners:
                    newVisitedCorners=[nextPosition]+visitedCorners
                    successor=((nextPosition,tuple(newVisitedCorners)),action,1)
...
def cornersHeuristic(state, problem):
    ...
    while unvisitedCorners:
        ...
        currentPosition=minCostCorner[1]
    return heuristic
...
def foodHeuristic(state, problem):
    ...
    if not unvisitedCorners:
        return 0
    dist=[(util.manhattanDistance(currentPosition,corner),corner) for corner in unvisitedCorners]
    minCostCorner=min(dist) 
    heuristic+=minCostCorner[0]
    dist=[(util.manhattanDistance(minCostCorner[1],corner),corner) for corner in unvisitedCorners]
    maxCostCorner=max(dist)
    heuristic+=maxCostCorner[0]
    return heuristic
